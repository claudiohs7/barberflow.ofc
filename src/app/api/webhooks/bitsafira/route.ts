...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!instanceId || !eventType) {
      console.warn("AVISO: Webhook recebido com payload invalido (faltando instancia ou evento).");
    ...
    switch (eventType) {
      case "qrcode.updated": {
        ...
        if (qrCodeBase64) {
          const cleanBase64 = qrCodeBase64.startsWith("data:image") ? qrCodeBase64.split(",")[1] : qrCodeBase64;
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      console.log(`INFO: Barbearia ${barbershop.id} atualizada com:`, Object.keys(updateData));
    }
