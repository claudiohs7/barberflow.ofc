...
export default function ClientMyAppointmentsPage() {
  ...
  const loadAppointments = useCallback(async () => {
    ...
    const phoneDigits = cleanPhone(clientProfile?.phone || user?.phone);
    ...
    try {
      ...
      const requests: Promise<{ data: Appointment[] }>[] = [];
      ...
      if (user?.id) {
        ...
        requests.push(fetchJson<{ data: Appointment[] }>(`/api/appointments?clientId=${user.id}`));
      ...
      if (phoneDigits && phoneDigits.length >= 10) {
        ...
        requests.push(fetchJson<{ data: Appointment[] }>(`/api/appointments?clientPhone=${phoneDigits}`));
      ...
      const rawAppointments = responses.flatMap((res) => res.data || []);
      ...
      const uniqueById = Array.from(
        ...
        rawAppointments.reduce<Map<string, Appointment>>((acc, appt) => {
          ...
          if (!acc.has(appt.id)) acc.set(appt.id, appt);
      ...
      const uniqueShopIds = Array.from(
        ...
        new Set(uniqueById.map((appt) => appt.barbershopId).filter((id): id is string => Boolean(id)))
  ...
  }, [user, clientProfile?.phone, toast]);
                      ...
                      <Card key={appt.id} className="flex flex-col">
                        ...
                        <CardHeader>
                          ...
                          {!canModify && (
                            ...
                            <div className="mt-2 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-center text-sm text-destructive">



                              Não é possível modificar ou cancelar com menos de 12h. Para cancelar, entre em contato com a barbearia pelo botão Contato.
