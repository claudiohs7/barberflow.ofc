...
const statusMeta: Record<WhatsAppStatus | "UNKNOWN", StatusMeta> = {
  ...
  AWAITING_SCAN: {
    ...
    description:
      "Gere/valide um QR e escaneie no WhatsApp > Aparelhos conectados.",
...
export default function WhatsAppPage() {
  ...
  const [isMissingInstanceDialogOpen, setIsMissingInstanceDialogOpen] =
    useState(false);

  const [qrGeneratedAt, setQrGeneratedAt] = useState<number | null>(null);

  const statusRef = useRef<WhatsAppStatus | null | undefined>(
    barbershop?.whatsappStatus,
  );

  const validateInFlightRef = useRef(false);

  const autoValidateTimerRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );

  const autoValidateAttemptsRef = useRef(0);
  ...
  const shouldShowQr =
    barbershop?.whatsappStatus !== "CONNECTED" && !!qrCodeUrl;
  ...
  const handleConnect = useCallback(async () => {
    ...
    try {
      ...
      setBarbershop((prev) =>
        prev
          ? {
              ...
              whatsappStatus:
                (json.whatsappStatus as WhatsAppStatus) || prev.whatsappStatus,
      ...
      toast({
        ...
        description:
          json.message || "Escaneie o QR Code no WhatsApp para concluir.",
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  const validateStatus = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      if (!barbershopId || validateInFlightRef.current) return;
      ...
      try {
        ...
        const nextQr =
          json.data?.qrCode || json.data?.qr || json.qrCodeBase64 || null;
        ...
        statusRef.current = nextStatus || currentStatus;
        ...
        if (!silent) {
          const isConnected = (nextStatus || currentStatus) === "CONNECTED";
          toast({
            ...
            description:
              json.message ||
              (isConnected
                ? "Conexao validada com o WhatsApp."
                : "Instancia esta desconectada."),
          });
        }
      } catch (error: any) {
        const msg: string = error?.message || "";
        const normalized = msg.toLowerCase();
        const missingInstanceError =
          normalized.includes("instancia bitsafira nao configurados") ||
          normalized.includes("instancia bitsafira nao configurados") ||
          normalized.includes("token ou id da instancia bitsafira");
        ...
        if (missingInstanceError) {
          ...
          console.warn("Validação automática do WhatsApp falhou:", msg);
        }
      } finally {
        if (!silent) {
  ...
  const handleValidateStatus = useCallback(() => {
    void validateStatus();
  }, [validateStatus]);
  ...
  useEffect(() => {
    const shouldAutoValidate =
      !!barbershopId &&
      !!barbershop?.bitsafiraInstanceId &&
      !!qrGeneratedAt &&
      statusRef.current !== "CONNECTED";

    if (!shouldAutoValidate) {
      autoValidateAttemptsRef.current = 0;
      if (autoValidateTimerRef.current) {
        clearInterval(autoValidateTimerRef.current);
    ...
    const intervalMs = 30_000; // espaça as chamadas de validação para cada 30s
    ...
    const tick = async () => {
      ...
      if (autoValidateAttemptsRef.current >= maxAttempts) {
        if (autoValidateTimerRef.current) {
          clearInterval(autoValidateTimerRef.current);
          autoValidateTimerRef.current = null;
        }
      ...
      autoValidateAttemptsRef.current += 1;
      await validateStatus({ silent: true });
    };

    void tick();
    autoValidateTimerRef.current = setInterval(() => {
      void tick();
    }, intervalMs);
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      setBarbershop((prev) =>
        prev
          ? { ...prev, whatsappStatus: "DISCONNECTED", qrCodeBase64: null }
          : prev,
      );
        ...
        <div className="flex h-[60vh] items-center justify-center">
          <Card className="max-w-xl">
            <CardHeader>
              ...
              <CardDescription>
                Atualize para Premium para habilitar automações e mensagens
                automáticas.
              </CardDescription>
            </CardHeader>
      ...
      <div className="flex h-[60vh] items-center justify-center">
        <Card className="max-w-xl">
          <CardHeader>
            ...
            <CardDescription>
              Não encontramos uma barbearia vinculada ao seu usuário. Crie ou
              selecione uma para habilitar o WhatsApp.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            Conecte sua instância no WhatsApp e personalize as mensagens
            automáticas da barbearia.
          </p>
        </div>
                ...
                <div className="rounded-lg border p-4">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge variant="outline">
                      {barbershop.bitsafiraInstanceId ? "Existente" : "Nova"}
                    </Badge>
                  </div>
                  ...
                  <ul className="space-y-2 text-sm text-muted-foreground list-disc pl-4">
                    <li>
                      Clique em "Conectar / Gerar QR" para criar ou recuperar a
                      instância.
                    </li>
                    ...
                    <li>
                      Depois de escanear, use "Validar status" para atualizar a
                      conexão.
                    </li>
                  </ul>
                </div>
                ...
                {shouldShowQr ? (
                  <div className="w-full space-y-3">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        shouldShowQr &&
                        barbershop?.whatsappStatus !== "CONNECTED" &&
                  ...
                  <div className="text-center text-sm text-muted-foreground space-y-2">
                    ...
                    <p>
                      Gere um QR Code para conectar ou valide o status da
                      instância já criada.
                    </p>

                    {statusInfo.label === "Conectado" && (
                      <p className="text-emerald-400">
                        Conexão ativa: não é necessário QR Code.
                      </p>
                    )}
                  </div>
                )}
      ...
      >
        <DialogContent className="max-w-md">
          ...
          {shouldShowQr ? (
            ...
            <p className="text-sm text-muted-foreground">
              Gere ou valide para exibir o QR Code.
            </p>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog
        open={isMissingInstanceDialogOpen}
        onOpenChange={setIsMissingInstanceDialogOpen}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Clique em <strong>Conectar</strong> para criar a instância
              BitSafira e gerar o QR Code.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-2">
            <Button
              variant="outline"
              onClick={() => setIsMissingInstanceDialogOpen(false)}
            >
              Fechar
            </Button>
