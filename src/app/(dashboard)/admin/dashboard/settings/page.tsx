...
export default function SettingsPage() {
      ...
      try {
        ...
        if (!data.error) {
          setFormState((prev) => {
            ...
            const unchanged =
              nextAddress.street === prev.address?.street &&
              nextAddress.neighborhood === prev.address?.neighborhood &&
              nextAddress.city === prev.address?.city &&
              nextAddress.state === prev.address?.state;
        ...
        toast({ variant: "destructive", title: "Erro de Rede", description: "N?o foi poss?vel buscar o endere?o." });
      }
    };
    void fetchAddress();
    return () => {
  ...
  }, [formCep, isEditingAddress, toast]);
...
const formCpfCnpj = formState.cpfCnpj || '';
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } else if (name === 'address.cep' || name === 'cep') {
